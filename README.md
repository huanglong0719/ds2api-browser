...
- **智能新对话检测** — 自动判断是否需要开启新对话（空闲超过 10 分钟，或当前对话消息数达到随机阈值 30-60 条）
...
| 当前对话消息数达到随机阈值（30-60 条） | 开启新对话 | 消息计数（`ShouldNewConversationByCount`） |
| 显式指定 `X-New-Conversation: true` | 强制新对话 | HTTP 头覆盖 |
| 显式指定 `?new=true` | 强制新对话 | URL 参数覆盖 |
| 配置 `auto_new_conversation: true` | 每次强制新对话 | 配置覆盖 |

> 注：自动新对话判断依据是**空闲时长**和**消息计数**，不检查 assistant 历史记录（README 旧版描述"无 assistant 历史"已不准确）。
