...
> 最后更新：2026-09-10（官方移除"快速/专家/识图"三模式，只剩深度思考+智能搜索开关）
...
### 模式切换（快速/专家/识图）
> ⚠️ **已过时（2026-09-10 探测确认）**：官方已将"快速/专家/识图"三模式合并为全新模型，`[role="radio"]` 及三模式字样均从新对话首页消失，本文结构仅作历史基线保留。当前新对话首页只剩"深度思考"和"智能搜索"两个开关。详见"九、变更记录"。
...
- **2026-09-10**：**探测确认官方已移除"快速/专家/识图"三模式（快速/专家/识图合并为全新模型）**，`模式切换（快速/专家/识图）` 章节已过时。证据（CDP 直连实测 + 今日日志）：
  - 通过临时新标签页实际打开新对话首页 `https://chat.deepseek.com/`，用 CDP `Runtime.evaluate` 全文扫描：`快速模式`、`专家模式`、`识图模式` 三个词计数**全为 0**；`[role="radio"]` 返回**空**（无任何模式 radio）。
  - 首页仅剩两个开关保留：**`深度思考`**、**`智能搜索`**（属 toggle，非模式切换，程序已有的 thinking/search toggle 检测不受影响）。
  - 今日日志佐证：**10:12 起**，每次开新对话 `getCurrentModeJS` 返回 `current mode: ""`（此时间点之前首页均返回 `"快速模式"`）。
  - **程序影响**：整套 `switchMode`/`switchToTextMode`/`switchToImageMode` 依赖已消失的 radio；文字请求当前不受影响（模式为空按快速处理），图片请求将来会因找不到"识图"radio 触发"强制回首页重试"乃至报错。改造（去模式切换，图片直接上传）已列待办，见 README。
